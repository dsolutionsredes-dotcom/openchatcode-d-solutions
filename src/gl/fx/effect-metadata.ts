import type { FxProperty } from "./uniforms.js";
...
export const EFFECT_METADATA: Record<string, EffectMetadata> = {
  "builtin:fx-luma-key": {
...
export function validateEffectOverrides(
  definition: Pick<EffectMetadata, 'props'> | undefined,
  raw: unknown,
): string | null {
  if (raw == null) return null;
  if (!definition || typeof raw !== 'object' || Array.isArray(raw)) return 'propertyOverrides must be an object for this effect';
  const props = new Map(definition.props.map((prop) => [prop.key, prop]));
  for (const [key, value] of Object.entries(raw as Record<string, unknown>)) {
    const prop = props.get(key);
    if (!prop) return `unknown effect parameter ${key}`;
    if (prop.kind === 'color') {
      if (!Array.isArray(value) || value.length !== 3 || !value.every((entry) => typeof entry === 'number' && Number.isFinite(entry) && entry >= 0 && entry <= 1)) {
        return `effect parameter ${key} must be an RGB array of three numbers between 0 and 1`;
      }
      continue;
    }
    if (typeof value !== 'number' || !Number.isFinite(value) || value < prop.min || value > prop.max) {
      return `effect parameter ${key} must be a finite number between ${prop.min} and ${prop.max}`;
    }
  }
  return null;
}
