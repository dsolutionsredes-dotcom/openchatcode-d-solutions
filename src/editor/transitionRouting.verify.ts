...
// 9:16 cover framing with a manual scale is rasterized before GLSL.
assert.deepEqual(
...
for (const type of ['circle-wipe', 'glitch-cut', 'clean-line-wipe', 'page-curl'] as const) {
  assert.deepEqual(
    routeVisualTransition(type, { ...video, transform: { scale: 2.05, x: -220, y: 0 } }, video),
    { renderer: 'gl' },
  );
}
