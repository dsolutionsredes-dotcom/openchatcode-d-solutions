...
export function isExplicitPreviewRequest(message: string): boolean {
  ...
  return /\b(?:preview|previsualizaci[oó]n)\b/.test(text) && !/\bno\s+(?:generes?|hacer|hagas?|crear|crees?)?\s*(?:un\s+)?(?:preview|previsualizaci[oó]n)\b/.test(text);
}
...
function activeState(doc: Awaited<ReturnType<typeof loadExternalProjectDoc>>): TimelineState {
  const timeline = doc?.timelines.find((entry) => entry && typeof entry === 'object' && (entry as { id?: unknown }).id === doc.activeTimelineId);
  ...
  const { jobId } = createGenerationJob(
    ...
    async (_jobId, update) => {
      ...
      await renderTimeline({
        ...
        onProgress: (progress: number) => update({ progress: 10 + Math.min(0.89, Math.max(0, progress)) * 89, processedFrames: Math.floor(frames * progress), totalFrames: frames }),
      });
      ...
      return { assetId: _jobId, kind: 'video' as const, name: filename, path: `/media/uploads/${filename}`, durationSeconds: frames / state.fps, sizeBytes: info.size, codec: 'h264' };
    },
...
export async function refreshExternalPreview(run: ExternalAgentRun): Promise<ExternalAgentRun> {
  ...
  if (!job) return run;
  if (job.status === 'failed') return { ...run, previewStatus: 'failed', preview: { ...run.preview, status: 'failed', error: job.error }, updatedAt: job.updatedAt };
  if (job.status === 'succeeded' && job.result?.path) return { ...run, previewStatus: 'ready', preview: { ...run.preview, status: 'ready', path: job.result.path, sizeBytes: job.result.sizeBytes }, updatedAt: job.updatedAt };
